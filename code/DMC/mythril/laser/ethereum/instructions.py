...
class Instruction:
    ...
    def push_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        value_duration = value_end_time - value_start_time
        
        # 如果总耗时大于5s，则打印每个部分的耗时
        if total_time > 5:
            print(f"========PUSH========")
            # 打印每个部分的耗时
            print(f"Total time: {total_time:.6f} seconds")
            print(f"Instruction fetch duration: {instruction_duration:.6f} seconds")
            print(f"Length calculation duration: {length_duration:.6f} seconds")
            print(f"Value processing duration: {value_duration:.6f} seconds")
    ...
    def add_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        op1 = helper.pop_bitvec(global_state.mstate)
        pop_end_time = time.time()
        
        # 记录第二个操作数的开始时间
        pop2_start_time = time.time()
        op2 = helper.pop_bitvec(global_state.mstate)
        pop2_end_time = time.time()
        ...
        pop2_duration = pop2_end_time - pop2_start_time
        ...
        if total_duration > 5:
            ...
            print(f"Pop first operand duration: {pop_duration:.6f} seconds")
            print(f"Pop second operand duration: {pop2_duration:.6f} seconds")
            print(f"Addition duration: {add_duration:.6f} seconds")
            print(f"Push result duration: {push_duration:.6f} seconds")
    ...
    def sub_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        # 记录第一个操作数的开始时间
        pop1_start_time = time.time()
        op1 = helper.pop_bitvec(global_state.mstate)
        pop1_end_time = time.time()
        ...
        # 计算每个部分的耗时
        pop1_duration = pop1_end_time - pop1_start_time
        pop2_duration = pop2_end_time - pop2_start_time
        ...
        if total_duration > 5:
            print(f"========SUB========")
            print(f"Total time: {total_duration:.6f} seconds")
            print(f"Pop first operand duration: {pop1_duration:.6f} seconds")
            print(f"Pop second operand duration: {pop2_duration:.6f} seconds")
            print(f"Subtraction duration: {sub_duration:.6f} seconds")
            print(f"Push result duration: {push_duration:.6f} seconds")
    ...
    def jumpi_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        try:
            ...
            print("##JUMPI:Skipping JUMPI to invalid destination.")
        ...
        if flag or negated_cond:
            ...
            print("##JUMPI:Pruned unreachable states.")
        ...
        if index is None:
            ...
            print("##JUMPI:Invalid jump destination: " + str(jump_addr))
            return states

        instr = disassembly.instruction_list[index]

        if instr["opcode"] == "JUMPDEST":
            if flag or positive_cond:
                new_state = deepcopy(global_state)
                # add JUMPI gas cost
                new_state.mstate.min_gas_used += min_gas
                new_state.mstate.max_gas_used += max_gas
                ...
                log.debug("Pruned unreachable states.")
                print("##JUMPI:Pruned unreachable states.2")
        return states
