...
def pop_bitvec(state: "MachineState") -> BitVec:
    ...
    # 记录布尔转换的开始时间
    bool_conversion_start_time = time.time()
    if isinstance(item, Bool):
        result = If(
            cast(Bool, item),
            symbol_factory.BitVecVal(1, 256),
        ...
        bool_conversion_end_time = time.time()
        total_time = time.time() - start_time
        pop_duration = pop_end_time - pop_start_time
        bool_conversion_duration = bool_conversion_end_time - bool_conversion_start_time
        if total_time > 2:
            print(f"=== Pop BitVec ===")
            print(f"Total time: {total_time:.6f} seconds")
            print(f"Pop duration: {pop_duration:.6f} seconds")
            print(f"Bool conversion duration: {bool_conversion_duration:.6f} seconds")
        return result
    
    # 记录整数转换的开始时间
    int_conversion_start_time = time.time()
    if isinstance(item, int):
        result = symbol_factory.BitVecVal(item, 256)
        int_conversion_end_time = time.time()
        total_time = time.time() - start_time
        pop_duration = pop_end_time - pop_start_time
        int_conversion_duration = int_conversion_end_time - int_conversion_start_time
        if total_time > 2:
            print(f"=== Pop BitVec ===")
            print(f"Total time: {total_time:.6f} seconds")
            print(f"Pop duration: {pop_duration:.6f} seconds")
            print(f"Int conversion duration: {int_conversion_duration:.6f} seconds")
        return result
    # 记录简化操作的开始时间
    else:
        item = cast(BitVec, item)
        ...
        total_time = time.time() - start_time
        pop_duration = pop_end_time - pop_start_time
        simplify_duration = simplify_end_time - simplify_start_time
        if total_time > 2:
            print(f"=== Pop BitVec ===")
            print(f"Total time: {total_time:.6f} seconds")
            print(f"Pop duration: {pop_duration:.6f} seconds")
            print(f"Simplify duration: {simplify_duration:.6f} seconds")
        return result
